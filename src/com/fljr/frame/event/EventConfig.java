package com.fljr.frame.event;

public interface EventConfig {
	/**
	 */
	public static final int EVENT_FINISH_FRAGMENT=0x3f421001;
	/**
	 */
	public static final int EVENT_START_FRAGMENT=0x3f421002;

	/**
	 */
	public static final int EVENT_CHANAGE_TITLE=0x3f421003;
	
	
	
	
	public static final String KEY_CLASS_NAME="KEY_CLASS_NAME";
	public static final String KEY_VALUE_BUNDLE="KEY_VALUE_BUNDLE";
	public static final String KEY_START_MODE="KEY_START_MODE";
	
	
	/**
	 */
	public static final int standard=1;
	public static final int singleTask=2;
	
	
}
